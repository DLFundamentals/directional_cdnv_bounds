...
class ScheduledCheckpoint(pl.Callback):
    def __init__(self, dirpath: str, early_every: int, early_until: int, late_every: int, save_last: bool = True, save_on_start: bool = True):
    ...
    def _ensure_dir(self, trainer: pl.Trainer, pl_module: pl.LightningModule):
        ...
        # Check if stage2 config exists, otherwise use stage1
        enabled = getattr(pl_module.cfg, 'stage2', None) is not None and pl_module.cfg.stage2.get('enabled', False)
        if enabled:
        ...
        # Only barrier if we're actually using a distributed strategy
    ...
    def _save_all_ranks(self, trainer: pl.Trainer, path: str):
        ...
        print(f"[ScheduledCheckpoint] Saving checkpoint to: {path}")
        trainer.save_checkpoint(path)
        # Only barrier if we're actually using a distributed strategy
        if hasattr(trainer.strategy, 'barrier'):
            trainer.strategy.barrier()
        print(f"[ScheduledCheckpoint] Checkpoint saved successfully")

    def on_fit_start(self, trainer: pl.Trainer, pl_module: pl.LightningModule):
        if not self.save_on_start:
            print("[ScheduledCheckpoint] Skipping initial checkpoint save (save_on_start=False)")
            return
