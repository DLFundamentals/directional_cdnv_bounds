...
from training_scratch.synthetic_dataloader import SyntheticShapesDataModule, SyntheticShapesCfg
...
def main():
    ...
    parser.add_argument('--out_csv', default='filename.csv', help='Output CSV file path')
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=sorted(LABEL_MAPS.keys()),
        help='(synthetic_shapes only) Override which attribute provides labels: one label_key per run',
    )   # FOR THE SYNTHETIC SHAPES DATASET
    parser.add_argument(
        '--num_classes',
        type=int,
        default=None,
        help='Override number of classes used by CDNV evaluator (defaults: cfg, else inferred for synthetic_shapes)',
    ...
    dataset_name = cfg.data.get('name', 'mini_imagenet')
    
    if dataset_name == 'synthetic_shapes':
        # For synthetic shapes
        data_config_dict = OmegaConf.to_container(cfg.data)
        expected_fields = {'data_root', 'metadata_file', 'img_size', 'batch_size', 'num_workers', 'num_views', 'val_fraction', 'seed', 'label_key'}
        filtered_config = {k: v for k, v in data_config_dict.items() if k in expected_fields}
        # Provide defaults if needed
        filtered_config.setdefault('data_root', './synthetic_shapes')
        filtered_config.setdefault('metadata_file', 'metadata.json')
        if args.label_key is not None:
            filtered_config['label_key'] = args.label_key
        data_cfg = SyntheticShapesCfg(**filtered_config)
        dm = SyntheticShapesDataModule(data_cfg)
    elif dataset_name == 'mini_imagenet':
        # For mini-imagenet
        data_config_dict = OmegaConf.to_container(cfg.data)
        expected_fields = {'name', 'hf_repo', 'hf_cache_dir', 'img_size', 'batch_size', 'num_workers', 'num_views', 'train_split', 'test_split'}
        filtered_config = {k: v for k, v in data_config_dict.items() if k in expected_fields}
        data_cfg = MiniImageNetCfg(**filtered_config)
    ...
    effective_label_key = None
    if dataset_name == 'synthetic_shapes':
        effective_label_key = getattr(dm.cfg, 'label_key', None)
        print(f"Synthetic label_key: {effective_label_key}")
    
    # Get number of classes
    cdnv_cfg = cfg.get('cdnv', {})
    cfg_num_classes = cfg.data.get('num_classes', None) or cdnv_cfg.get('num_classes', None)
    if args.num_classes is not None:
        num_classes = args.num_classes
    elif cfg_num_classes is not None:
        num_classes = int(cfg_num_classes)
    elif dataset_name == 'synthetic_shapes' and effective_label_key in LABEL_MAPS:
        num_classes = len(LABEL_MAPS[effective_label_key])
    else:
        num_classes = 100
    ...
    for epoch, ckpt_path in files:
        ...
        try:
            ...
            traceback.print_exc()
            rows.append({'epoch': epoch, 'label_key': effective_label_key, 'error': str(e)})
